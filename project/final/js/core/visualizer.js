 ...
 * Fixed implementation addresses analyzer node access and canvas initialization issues.
 */
...
function setupCanvas() {
    ...
    // Check if the waveform viewer element exists
    if (!waveformViewer) {
        console.error('Waveform viewer element not found. Visualization disabled.');
        return;
    }
...
function addVisualizationControls() {
    ...
    if (!waveformContainer) {
        console.error('Waveform container not found. Visualization controls disabled.');
        return;
    }
...
function showModeChangeFeedback(container) {
    ...
    // Animation is handled by CSS in animations.css
 ...
 * Fixed to properly handle audio context initialization
 */
function initAnalyzer() {
    // Check if we already have an analyzer
    if (isVisualizerInitialized) return;
    
    // Wait for audio context to be available
    if (!window.audioContext) {
        console.log('Audio context not available yet. Will retry later.');
        return;
    }
    ...
    if (typeof getAnalyser === 'function') {
        ...
        if (analyser) {
            console.log('Successfully obtained analyzer from sequencer');
        }
    }
    
    // If still no analyzer, create one
    if (!analyser) {
        console.log('Creating new analyzer node');
        ...
        try {
            ...
            analyzerGain.connect(window.audioContext.destination);
            
            console.log('Created and connected new analyzer node');
        } catch (error) {
            console.error('Error connecting analyzer node:', error);
        }
    }
    ...
    console.log('Visualizer analyzer initialized');
}
...
function updateVisualizer(step) {
    ...
    // If still no analyzer or canvas, exit early
    if (!analyser || !canvas) {
        return;
    }
...
function drawWaveform(transitionProgress = 1) {
    ...
    const color = getComputedStyle(document.documentElement).getPropertyValue('--color-magenta') || '#FF0097';
...
function drawDefaultWaveform() {
    ...
    canvasContext.strokeStyle = 'rgba(255, 0, 151, 0.3)'; // Magenta with opacity
    canvasContext.stroke();
}
...
function drawDefaultBars() {
    ...
    for (let i = 0; i < barCount; i++) {
        ...
        canvasContext.fillStyle = 'rgba(255, 0, 151, 0.3)';
...
function drawDefaultCircles() {
    ...
    for (let i = 5; i > 0; i--) {
        ...
        canvasContext.beginPath();
        canvasContext.arc(centerX, centerY, radius, 0, Math.PI * 2);
        canvasContext.strokeStyle = `rgba(255, 0, 151, ${0.1 + (5-i) * 0.05})`;
        canvasContext.lineWidth = 2;
        canvasContext.stroke();
    }
